...
for ii = 1:length(z)
    ...
    Iop = np0*e0*c0/2*abs((ifft(Aop.*exp(-1i*(k_omega-k_omega0)*z(ii)).'))*omegaMAX).^2;
    subplot(2,4,2)
    plot(t*1e12, 1e-13*Iop);
    xlim([-5 5]);
    ...
    title('SH-Efficiency (%)');
    xlabel('Crystal length (mm)');
    ETHz = real((ifft(FA.'.*ATHz.*exp(-1i*(k_OMEGA-k_OMEGA0)*z(ii)).')))*omegaMAX;
    subplot(2,4,8)
    plot(t*1e12,1e-5*ETHz);
    ...
    %xlim([0 5]);
    %title('THz refractive index');
    ISH = abs((ifft(ASH.*exp(-1i*(k_omegaSH-k_omegaSH0)*z(ii)).'))*omegaMAX).^2;
    subplot(2,4,4);
    plot(t*1e12, 1e-13*neo(lambda0/2,T,cry)*e0*c0/2*ISH);
    xlim([-5 5]);
    %xlim([omega0-omega0/10,omega0+omega0/10]/2/pi);
    title('SH Intensity');
    xlabel('Time (ps)');
    ...
    % end;
    DataBaseWriter("./"+dir_n+"/DB",z(ii),abs(Aop),Iop,abs(ATHz),ETHz,abs(ASH),ISH);
end
...
DataBaseEnder("./"+dir_n+"/DB",z,t,omega/2/pi,effic,efficSH);
%end
